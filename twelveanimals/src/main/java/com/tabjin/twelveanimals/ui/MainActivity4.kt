...
import com.tabjin.twelveanimals.adapter.Type4Adapter
import com.tabjin.twelveanimals.bean.Type
import com.tabjin.twelveanimals.model.CompainModel
import com.tabjin.twelveanimals.model.Constant
...
class MainActivity4 : AppCompatActivity(), View.OnClickListener {
    override fun onClick(v: View?) {
        when(v?.id){
            ...
            R.id.btn_start ->{
                ...
                var condition = java.lang.StringBuilder("$chuhaoStr\n胆码1：")
                ...
                //所有码集合
                val all = ArrayList<String>(Constant.allAnimals)
                if(chuhaoList.isEmpty()){
                    Toast.makeText(this@MainActivity4,"请选择任几",Toast.LENGTH_SHORT).show()
                    return
                }
                ...
                for(a in chu) {
                    if (renji!! < a) {
                        Toast.makeText(this@MainActivity4, "出码数不能大于任几数", Toast.LENGTH_SHORT).show()
                        return
                    }
                }
                if(chuList1.isEmpty()){
                ...
                Thread{
                    ...
                    runOnUiThread{
                        dismissPD()
                        val intent = Intent(this@MainActivity4, ResultActivity::class.java)
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        recycler.addItemDecoration(object:RecyclerView.ItemDecoration(){
            override fun getItemOffsets(outRect: Rect, view: View, parent: RecyclerView, state: RecyclerView.State) {
                ...
                if(parent.getChildAdapterPosition(view) == 0){
                    outRect.top = dip2px(this@MainActivity4,20f)
                }
            }
        })
        ...
        recycler.addItemDecoration(object :RecyclerView.ItemDecoration(){
            override fun getItemOffsets(outRect: Rect, view: View, parent: RecyclerView, state: RecyclerView.State) {
                super.getItemOffsets(outRect, view, parent, state)
                outRect.bottom = dip2px(this@MainActivity4,20f)
            }
        })
        initData()
    ...
    fun initData(){
        try {
            var input = assets.open("type4.json")
